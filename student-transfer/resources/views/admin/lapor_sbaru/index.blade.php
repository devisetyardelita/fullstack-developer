...
<div class="content">
  <div class="container-xxl flex-grow-1 container-p-y">
    <h4 class="mb-2">Data Layanan Mutasi Siswa Masuk</h4>
    @if (session('status'))
      ...
      {{-- <h3 class="fw-bold mb-3" style="color: white">Data Layanan Mutasi Siswa Masuk</h3> --}}
        ...
        <div class="d-flex align-items-center">
          ...
          <div class="navbar-nav align-items-left me-2" style="width: 75%;"> <!-- Atur lebar search -->
            <form action="{{ route('admin.sbaru') }}" method="GET">
        ...
        <div class="divider d-flex justify-content-center align-items-center mt-0">
          ...
          <a href="{{ url('admin/mutasi_masuk/excel') }}" class="btn rounded-pill btn-primary mx-1">Excel</a>
          <a href="{{ url('admin/mutasi_masuk/pdf') }}" class="btn rounded-pill btn-secondary mx-1">PDF</a>
          {{-- <a href="{{ route('admin.sbaru.create') }}" class="btn rounded-pill btn-primary mx-1">
              ...
              @else
                <table class="table table-bordered">
                  <thead>
                    <tr>
                      <th>
                        <a href="{{ route('admin.sbaru', ['sort_by' => 'id', 'sort_order' => $sortBy == 'id' && $sortOrder == 'asc' ? 'desc' : 'asc', 'search' => $search]) }}">
                            No.
                            @if ($sortBy == 'id')
                                @if ($sortOrder == 'asc')
                      ...
                      <th>
                          <a href="{{ route('admin.sbaru', ['sort_by' => 'nama_siswa', 'sort_order' => $sortBy == 'nama_siswa' && $sortOrder == 'asc' ? 'desc' : 'asc', 'search' => $search]) }}">
                              Nama Siswa
                      ...
                      <th>
                          <a href="{{ route('admin.sbaru', ['sort_by' => 'kelas', 'sort_order' => $sortBy == 'kelas' && $sortOrder == 'asc' ? 'desc' : 'asc', 'search' => $search]) }}">
                              Kelas
                              @if ($sortBy == 'kelas')
                                  @if ($sortOrder == 'asc')
                                      <i class="bx bx-sort-up"></i>
                      ...
                      <th>
                          <a href="{{ route('admin.sbaru', ['sort_by' => 'school_sbaru_id', 'sort_order' => $sortBy == 'school_sbaru_id' && $sortOrder == 'asc' ? 'desc' : 'asc', 'search' => $search]) }}">
                              Sekolah Tujuan
                  ...
                  <tbody id="tbody">                         
                    @foreach($sbaru as $item)
                      <tr>
                        ...
                        <td>
                          <a href="{{ route('admin.sbaru.show', ['tipe' => $item->tipe == 'dalam' ? 'dalam' : 'luar', 'id' => $item->id]) }}" class="btn btn-dark btn-sm">Detail
...
<script>
  $(document).ready(function(){
    $('#applyFilter').on('click', function(){
      ...
      $.ajax({
        url: "{{ route('admin.sbaru.filter') }}", // Adjust the route to match your filter route
        type: "GET",
          ...
          if (filteredStatuses.length > 0) {
            for (let i = 0; i < filteredStatuses.length; i++) {
              ...
              html += `<tr>
                ...
                <td>${filteredStatuses[i].slama}</td>
                <td>
                  <span class="badge ${badgeClass} me-1">${filteredStatuses[i].status}</span>
                </td>
                <td>
                  <a href="/admin/sbaru/show/${filteredStatuses[i].id}" class="btn btn-dark btn-sm">Detail
                    <i class="bx bx-right-arrow-alt"></i>
                  </a>
                </td>
              </tr>`;
