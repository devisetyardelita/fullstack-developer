<form action="{{ route('user.mutasi_siswa_luar.store') }}" method="POST" enctype="multipart/form-data">
    @csrf
    ...
    <!-- Sekolah Tujuan -->
    <div class="form-group mb-3">
        <label for="school_sbaru_name" class="fw-bold mb-1">Sekolah Tujuan</label>
        <input type="text" class="form-control rounded-pill @error('school_sbaru_name') is-invalid @enderror" id="school_sbaru_name" name="school_sbaru_name" placeholder="Masukkan Nama Sekolah Anda..." value="{{ old('school_sbaru_name') }}" autofocus/>
    ...
    <!-- Surat Asal-->
    <div class="form-group mb-3">
        <label for="surat_slama" class="fw-bold mb-1">Surat Rekomendasi Mutasi Sekolah Asal</label>
        <input type="file" id="surat_slama" name="surat_slama" class="form-control rounded-pill @error('surat_slama') is-invalid @enderror" />
        <small style="color:gray"><p>File: pdf, jpg, jpeg, png, doc, docx.</p></small>
          @error('surat_slama')
        <div class="invalid-feedback">{{ $message }}</div>
         @enderror
