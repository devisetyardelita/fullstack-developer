      ...
      <h3 class="fw-bold mb-3" style="color: white">Data Layanan Mutasi Siswa</h3>
        ...
        <div class="d-flex align-items-center">
          ...
          <div class="navbar-nav align-items-left me-2" style="width: 75%;"> <!-- Atur lebar search -->
            <form action="{{ route('operator.mutasi_siswa') }}" method="GET">
        ...
        <div class="divider d-flex justify-content-center align-items-center mt-0">
          ...
          <a href="{{ route('operator.mutasi_siswa.create') }}" class="btn rounded-pill btn-primary mx-1">
        ...
        <div class="card">
          <div class="card-body">
            <div class="table-responsive text-nowrap">
              <table class="table table-bordered">
                <thead>
                  <tr>
                    <th>
                      <a href="{{ route('operator.mutasi_siswa', ['sort_by' => 'id', 'sort_order' => $sortBy == 'id' && $sortOrder == 'asc' ? 'desc' : 'asc', 'search' => $search]) }}">
                          No.
                          @if ($sortBy == 'id')
                              @if ($sortOrder == 'asc')
                  ...
                  <th>
                      <a href="{{ route('operator.mutasi_siswa', ['sort_by' => 'nama_siswa', 'sort_order' => $sortBy == 'nama_siswa' && $sortOrder == 'asc' ? 'desc' : 'asc', 'search' => $search]) }}">
                          Nama
                  ...
                  <th>
                      <a href="{{ route('operator.mutasi_siswa', ['sort_by' => 'no_hp', 'sort_order' => $sortBy == 'no_hp' && $sortOrder == 'asc' ? 'desc' : 'asc', 'search' => $search]) }}">
                          No. HP/WA
                          @if ($sortBy == 'no_hp')
                              @if ($sortOrder == 'asc')
                                  <i class="bx bx-sort-up"></i>
                ...
                <tbody id="tbody">                         
                  @foreach($mutasi_siswa as $item)
                    <tr>
                      ...
                      <td>
                        <a href="{{ url('admin/mutasi_siswa/edit/' . $item->id) }}" class="btn btn-secondary btn-sm">
                          ...
                          <div class="modal-dialog modal-sm">
                            <div class="modal-content text-center p-4">
                              ...
                              <div class="d-flex justify-content-center gap-3 mb-2">
                                ...
                                <form action="{{ url('admin/mutasi_siswa/delete/' . $item->id) }}" method="POST" class="d-inline">
                                    @method('delete')
                                    @csrf
                        ...
                        <a href="{{ url('admin/mutasi_siswa/show/' . $item->id) }}" class="btn btn-dark btn-sm">Detail
            ...
            <div class="d-flex justify-content-end mt-2">
              {{ $mutasi_siswa->links('pagination::bootstrap-5') }}
            </div>
          </div>
        </div>
        <!--/ Bordered Table -->
...
<script>
  $(document).ready(function(){
    $('#applyFilter').on('click', function(){
      ...
      $.ajax({
        url: "{{ route('operator.mutasi_siswa.filter') }}", // Adjust the route to match your filter route
        type: "GET",
