...
<div class="content">
    <div class="container-xxl flex-grow-1 container-p-y">
      ...
      @endif
      <h3 class="fw-bold mb-3" style="color: white">Data Layanan Pengaduan Tidak Langsung</h3>

          <!-- Search and Filter Container -->
          <div class="d-flex align-items-center">
            <!-- Search -->
            <div class="navbar-nav align-items-left me-2" style="width: 75%;"> <!-- Atur lebar search -->
              <form action="{{ route('pengaduan_tidak_langsung') }}" method="GET">
          ...
          <!-- /Search and Filter Container -->
            ...
            <div class="divider d-flex justify-content-center align-items-center mt-0">
              <a href="{{ url('admin/pengaduan_tidak_langsung/excel') }}" class="btn rounded-pill btn-primary mx-1">Excel</a>
              <a href="{{ url('admin/pengaduan_tidak_langsung/pdf') }}" class="btn rounded-pill btn-secondary mx-1">PDF</a>
              <a href="{{ route('pengaduan_tidak_langsung.create') }}" class="btn rounded-pill btn-primary mx-1">
                ...
                <div class="card-body">
                  <div class="table-responsive text-nowrap">
                    <table class="table table-bordered">
                      ...
                      <tbody id="tbody">                         
                        @foreach($pengaduan_tidak_langsung as $item)
                          <tr>

                            <td>{{ $loop->iteration + $pengaduan_tidak_langsung->firstItem() - 1 }}</td>
                            ...
                            <td>
                                <a href="{{ url('admin/pengaduan_tidak_langsung/edit/' . $item->id) }}" class="btn btn-secondary btn-sm">
                                  ...
                                  <div class="modal-dialog modal-sm">
                                    <div class="modal-content text-center p-4">
                                      ...
                                      <div class="d-flex justify-content-center gap-3 mb-2">
                                        ...
                                        <form action="{{ url('admin/pengaduan_tidak_langsung/delete/' . $item->id) }}" method="POST" class="d-inline">
                                            @method('delete')
                                            @csrf
                                ...
                                <a href="{{ url('admin/pengaduan_tidak_langsung/show/' . $item->id) }}" class="btn btn-dark btn-sm">Detail
                  ...
                  <div class="mt-2">
                    {{ $pengaduan_tidak_langsung->links() }}
                  </div>
                </div>
              </div>
              <!--/ Bordered Table -->
...
<script>
  $(document).ready(function(){
    $('#applyFilter').on('click', function(){
      ...
      $.ajax({
        url: "{{ route('pengaduan_tidak_langsung.filter') }}", // Adjust the route to match your filter route
        type: "GET",
          ...
          if (filteredStatuses.length > 0) {
            for (let i = 0; i < filteredStatuses.length; i++) {
              ...
              html += `<tr>
                ...
                <td>
                  <a href="/admin/pengaduan_tidak_langsung/edit/${filteredStatuses[i].id}" class="btn btn-secondary btn-sm">
                    <i class="bx bx-edit-alt me-1"></i>
                  </a>
                  <form action="/admin/pengaduan_tidak_langsung/delete/${filteredStatuses[i].id}" method="POST" class="d-inline">
                    @method("delete")
                    @csrf
                  ...
                  <a href="/admin/pengaduan_tidak_langsung/show/${filteredStatuses[i].id}" class="btn btn-dark btn-sm">Detail
                    <i class="bx bx-right-arrow-alt"></i>
                  </a>
                </td>
              </tr>`;
